...
All the files/scripts for SALT3 training can be found in the SALT3_training directory
...
containing a random MAGSHIFT/WAVESHIFT combination for the relevant survey/band. These are the files referenced later under the TRAINOPT block of Submit_trainsalt3.config. 



## 1.2 GENERATING SALT3 MODEL TRAINING DATASETS (Light curves as well as prism spectra)

To simulate LSST SNe (see further instructions in SIMGEN_LSST.input), do

```bash
  snlc_sim.exe SIMGEN_LSST.input   
```

To simulate Roman SNe (both light curves and prism spectra - further instructions in SIMGEN_Roman.input)

```bash
snlc_sim.exe SIMGEN_Roman.input   
```
...
that both extractor files can take one or more surveys and give a combined SN_peak_MJD.list and SN_params.list for the surveys. The extractor scripts also reference fitres_utils
...
Before running Submit_trainsalt3.config, ensure that:
  ...
  d. The right conda environment is activated (usually SALTShaker_kene_mod or any other SALTShaker environment): 
  ```bash
         conda activate SALTShaker_kene_mod   
  ```
